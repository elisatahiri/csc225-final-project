...
async function fetchCharacterByIndex(index) {
    try {
        ...
        const character = characters.find(char => char.index == index);
...
async function init() {
    ...
    if (!characterIndex) {
        showError('No character specified. Please go back and select a character.');
        return;
    }
    
    try {
        const character = await fetchCharacterByIndex(characterIndex);
